 ...
 * @author shenbowei
 */
func (this *SSHSession) ReadChannelExpect2(timeout time.Duration, expects ...string) string {
...
func (this *SSHSession) ReadChannelRegExp(timeout time.Duration, re string) string {
...
func (this *SSHSession) ReadChannel(timeout time.Duration, fn func(string) bool) string {
	LogDebug("ReadChannel: <wait timeout = %d>", timeout/time.Millisecond)
	output := ""
	isDelayed := false
	...
	for i := 0; i < 300; i++ { //最多从设备读取300次，避免方法无法返回
		time.Sleep(time.Millisecond * 100) //每次睡眠0.1秒，使out管道中的数据能积累一段时间，避免过早触发default等待退出
		newData := this.readChannelData()
		LogDebug("ReadChannel: read chanel buffer: %s", newData)
		if newData != "" {
			output += newData
			isDelayed = false
			continue
		...
		//如果之前已经等待过一次，则直接退出，否则就等待一次超时再重新读取内容
		if !isDelayed {
			LogDebug("ReadChannel: delay for timeout.")
			time.Sleep(timeout)
			isDelayed = true
		} else {
			return output
